...
export class Repository implements IRepository {
    ...
    //创建版本控制的语句
    private static CREATE_VERSION_TABLE: string = "CREATE TABLE IF NOT EXISTS VERSION_ (VALUE_ INT NOT NULL)";

    //初始化版本控制的语句
    private static INIT_VERSION_TABLE_CONTENT: string = "INSERT INTO VERSION_ (VALUE_) values (?) ";

    //查询当前的版本号
    private static QUERY_CURRENT_VERSION: string = "SELECT VALUE_ FROM VERSION_ LIMIT 1";

    //更新版本SQL
    private static UPDATE_VERSION: string = "UPDATE VERSION_ SET VALUE_ = ?";
    ...
    private prepareDB() {
        ...
        const userDataPath = FSDirUtil.documentPath();
